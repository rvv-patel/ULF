...
import { addApplicationDocument, updateApplicationDocument } from '../../../store/slices/applicationDocumentSlice';
import type { ApplicationDocument } from './types';

export default function ApplicationDocumentFormPage() {
    const { id } = useParams();
    ...
    const existingItem = useAppSelector(state =>
        state.applicationDocument.items.find(i => i.id === Number(id))
    );

    const [formData, setFormData] = useState<Partial<ApplicationDocument>>({
        title: existingItem?.title || '',
        documentFormat: existingItem?.documentFormat || '.pdf',
        isUploaded: existingItem?.isUploaded || false,
        isGenerate: existingItem?.isGenerate || false
    });
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isEditMode) {
                await dispatch(updateApplicationDocument(payload)).unwrap();
            } else {
                await dispatch(addApplicationDocument(payload)).unwrap();
            }
            navigate('/masters/application-documents');
        } catch (err) {
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 w-full max-w-2xl p-8">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-gray-900">
                        {isEditMode ? 'Edit Application Document' : 'Add New Application Document'}
                    </h1>
                </div>
